...
int get_student(int fd, int id, student_t *s){
    // Compute the byte offset in the file for student with this 'id'
    ...
    // Attempt to read exactly one student record
    student_t temp = {0};
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    // Read existing record to check if it is empty
    student_t tmp = {0};
    ssize_t bytes_read = read(fd, &tmp, STUDENT_RECORD_SIZE);
    if (bytes_read < 0) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if (bytes_read == STUDENT_RECORD_SIZE) {
        // Check if this record is already in use
    ...
    // Create a student_t to write
    student_t new_stud = {0};
    new_stud.id = id;
    strncpy(new_stud.fname, fname, sizeof(new_stud.fname) - 1);
    strncpy(new_stud.lname, lname, sizeof(new_stud.lname) - 1);
    new_stud.gpa = gpa;
    ...
    // write the record
    ssize_t bytes_written = write(fd, &new_stud, STUDENT_RECORD_SIZE);
...
int del_student(int fd, int id){
    ...
    // write an EMPTY_STUDENT_RECORD at that location
    off_t offset = (off_t)id * STUDENT_RECORD_SIZE;
    off_t pos = lseek(fd, offset, SEEK_SET);
    if (pos == (off_t)-1) {
        printf(M_ERR_DB_READ);
...
int print_db(int fd){
    ...
    while (1) {
        ...
        if (memcmp(&tmp, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            ...
            float real_gpa = tmp.gpa / 100.0f;
            printf(STUDENT_PRINT_FMT_STRING, 
                   tmp.id, tmp.fname, tmp.lname, real_gpa);
        }
    }
...
int compress_db(int fd){
    // Return to start of old DB
    if (lseek(fd, 0, SEEK_SET) == (off_t)-1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    //Open the temporary DB file
    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP;
    ...
    //Copy student records from old DB to temp DB
    student_t tmp;
    while (1) {
        ssize_t bytes_read = read(fd, &tmp, STUDENT_RECORD_SIZE);
        if (bytes_read < 0) {
        ...
        // if valid record, copy
        if (memcmp(&tmp, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            // compute offset
            off_t new_offset = (off_t)tmp.id * STUDENT_RECORD_SIZE;
            if (lseek(temp_fd, new_offset, SEEK_SET) == (off_t)-1) {
                printf(M_ERR_DB_READ);
                close(temp_fd);
                return ERR_DB_FILE;
            }
            ssize_t written = write(temp_fd, &tmp, STUDENT_RECORD_SIZE);
    ...
    close(fd);
    // flush and close temp
    close(temp_fd);

    // Rename the temp file to be the real DB_FILE
    //    remove the old DB_FILE first
    unlink(DB_FILE);
    ...
    // reopen the new DB_FILE and return its fd
    int new_fd = open(DB_FILE, O_RDWR, mode);
    if (new_fd < 0) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }
