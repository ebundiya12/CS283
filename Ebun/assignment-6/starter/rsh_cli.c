...
int exec_remote_cmd_loop(char *address, int port)
{
...

    {
        ...
        if (!fgets(cmd_buff, SH_CMD_MAX, stdin)) {
            printf("\n");
            // treat as user typed 'exit'
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, OK);
        }
        // remove trailing newline
        cmd_buff[strcspn(cmd_buff, "\n")] = '\0';

        // send null-terminated command to server
        int send_len = strlen(cmd_buff) + 1; 
        ssize_t s = send(cli_socket, cmd_buff, send_len, 0);
        if (s < send_len) {
            perror("send");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        }

        // If user typed exit or stop-server => done
        if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, OK);
        }

        // read server response until we see RDSH_EOF_CHAR
        while (1) {
            // Store recv() result in io_size
            ...
            // check if the last byte is EOF_CHAR
