...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        int idx = 0;
        while (1) {
            ssize_t n = recv(cli_socket, &io_buff[idx], 1, 0);
            io_size = n;
        ...
        // check if command is "exit" or "stop-server"
        if (strcmp(io_buff, "exit") == 0) {
            free(io_buff);
            return OK; // done with this client
        }
        if (strcmp(io_buff, "stop-server") == 0) {
            free(io_buff);
            return OK_EXIT; // server shuts down
        }

        // parse into command_list_t (which has array of cmd_buff_t)
        memset(&cmd_list, 0, sizeof(cmd_list));
        rc = build_cmd_list(io_buff, &cmd_list);
        if (rc != OK) {
            send_message_string(cli_socket, "rdsh-error: parse error\n");
            continue;
        }

        // If there's exactly 1 command in the pipeline, check built-ins like "cd" or "dragon"
        if (cmd_list.num == 1) {
            // The single cmd is in cmd_list.commands[0] -> a cmd_buff_t
            cmd_buff_t *first_cmd = &cmd_list.commands[0];
            // first_cmd->argv[0] is the command name
            if (first_cmd->argc > 0) {
                char *exe_name = first_cmd->argv[0];

                // "cd"
                if (strcmp(exe_name, "cd") == 0) {
                    // if there's an argument
                    if (first_cmd->argc > 1) {
                        char *dir = first_cmd->argv[1];
                        if (chdir(dir) != 0) {
                            send_message_string(cli_socket, "cd failed\n");
                        } else {
                            send_message_string(cli_socket, "cd ok\n");
                        }
                    } else {
                        send_message_string(cli_socket, "cd does nothing\n");
                    }
                    continue;
                }

                // "dragon"
                if (strcmp(exe_name, "dragon") == 0) {
                    send_message_string(cli_socket, "[ASCII Dragon goes here]\n");
                    continue;
                }
            }
        }
        ...
        // let client know output is done
...
int send_message_string(int cli_socket, char *buff){
    ...
    int len = strlen(buff);
    ssize_t s = send(cli_socket, buff, len, 0);
    if (s < len){
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pid == 0) {
            ...
            // Connect STDIN
            if (i == 0) {
                dup2(cli_sock, STDIN_FILENO);        // first command reads from cli_sock
            } else {
                dup2(pipes[i-1][0], STDIN_FILENO);   // otherwise from previous pipe
            }

            // Connect STDOUT/ERR
            if (i == clist->num - 1) {
                // last command writes to cli_sock
                dup2(cli_sock, STDOUT_FILENO);
                dup2(cli_sock, STDERR_FILENO);
            } else {
                // otherwise to next pipe
            ...
            // Build local argv from cmd_buff_t->argv
            cmd_buff_t *this_cmd = &clist->commands[i];
            char *exec_argv[64];
            memset(exec_argv, 0, sizeof(exec_argv));

            // copy up to 63 arguments
            for (int a = 0; a < this_cmd->argc && a < 63; a++) {
                exec_argv[a] = this_cmd->argv[a];
            }
            exec_argv[this_cmd->argc] = NULL;
